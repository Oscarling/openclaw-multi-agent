...
- [ ] 执行 `rh_t5_b01_route_parity_remediation_requested`（路由口径整改）
  ...
  尝试记录：`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a1.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a2-script-hardening.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a3-channel-last.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a4-shadow-2026-3-24.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a5-binding-hypothesis.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a6-min-repro-pack.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a7-controlled-comparison.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-remediation-attempt-a8-to-path-probe.md`、`design/validation/2026-03-28-role-hardening-rh-t5-b01-upstream-issue-escalation.md`、`design/validation/2026-03-28-rh-t5-b01-upstream-recheck-bundle-baseline.md`、`design/validation/2026-03-28-rh-t5-b01-upstream-feedback-probe-baseline.md`、`design/validation/2026-03-28-rh-t5-b01-event-runner-baseline.md`
  外部协作：本地 issue `#37` + 上游 issue `openclaw/openclaw#56370`（`#56267` 已关闭）；PR `#38/#39/#40/#41`
  触发卡：`design/validation/2026-03-28-rh-t5-b01-upstream-feedback-trigger-card-v1.md`
  当前状态：A1/A3/A4/A5/A6/A7/A8 均未消除分裂；A2 完成关键脚本护栏硬化；A7 已确认分裂与 provider/model/profile 受控对比无关；A8 已确认分裂集中在 `--to` 路径；A9 已完成上游问题单升级；A10 已完成上游反馈复检包基线留痕（`blocker_close_ready=no`）；A11 已完成上游反馈探针基线留痕（`next_event=waiting_upstream_feedback`），本地追踪 issue #37 已纠偏重开；A12 已完成事件执行器基线留痕（`action_taken=wait`）；A13 已完成事件执行器复检（`next_event=waiting_upstream_feedback`、`action_taken=wait`）；A14 已完成上游跟进评论补充（等待 maintainer 反馈）；A15 已完成 `agent_call_guard` 覆盖面补强（`deploy + scripts` 门禁）；A16 已完成 A15 后状态一致性复检（`next_event=waiting_upstream_feedback`、`action_taken=wait`）；A17 已完成“上游反馈水位线去重”能力（仅新反馈触发复检包）；A18 已完成 gstack 双专家评估（`office-hours + plan-eng-review`，结论一致为 `Conditional-Go`）；A19 已完成“治理行动卡”固化（事件分流、等待态动作、关单门槛统一）；A20 已完成上游 maintainer handoff v2（最短复现 + 判定口径）；A21 已完成 gstack 专家再评审（根因 + 解法），结论保持 `Conditional-Go` 且关闭门槛不变；A22 已完成 post-A21 事件执行器复检（重试后 `next_event=waiting_upstream_feedback`、`upstream_new_feedback_detected=no`）；A23 已完成并行主链策略落地（`P5` 侧线继续开启，新增并行主线分支推进）；A24 已完成上游 issue 切换默认监控验证（默认 `upstream_issue_number=56370`）
  最新记录：`design/validation/2026-03-28-rh-t5-b01-event-runner-manual-continue-validation.md`、`design/validation/2026-03-28-rh-t5-b01-upstream-followup-comment-validation.md`、`design/validation/2026-03-28-rh-t5-b01-agent-call-guard-scope-hardening-validation.md`、`design/validation/2026-03-28-rh-t5-b01-event-runner-post-a15-validation.md`、`design/validation/2026-03-28-rh-t5-b01-feedback-watermark-validation.md`、`design/2026-03-28-rh-t5-b01-gstack-dual-expert-assessment-minutes-v1.md`、`design/validation/2026-03-28-rh-t5-b01-governance-action-card-v1.md`、`design/validation/2026-03-28-rh-t5-b01-upstream-maintainer-handoff-v2.md`、`design/2026-03-28-rh-t5-b01-gstack-root-cause-solution-rereview-minutes-v1.md`、`design/validation/2026-03-28-rh-t5-b01-event-runner-post-a21-validation.md`、`design/validation/2026-03-28-rh-t5-b01-parallel-mainline-policy-v1.md`、`design/validation/2026-03-28-rh-t5-b01-upstream-issue-rollover-validation.md`

- [x] 启动“P5 阻断并行主链”执行口径（A23）
  触发条件：用户确认“`RH-T5-B01` 不关单但允许并行推进其它主链”（已满足）
  完成标准：形成并行主链策略卡并切出独立并行分支，且不改变 `RH-T5-B01` 关闭门槛
...
- [x] 完成 `xhs_demo_002` Stage B 发布回执并通过严格复检（A30）
...
- [x] 完成并行主链多账号自动登录实现准备包 v2（A37）
  ...
  实现准备包：`design/2026-03-28-parallel-mainline-multi-account-autologin-implementation-prep-v2.md`
  事件执行卡：`design/2026-03-28-parallel-mainline-multi-account-autologin-event-card-v1.md`
...
- [x] 固化并行主链多账号自动登录受控窗口运行口径（A42）
  ...
  运行手册：`design/2026-03-29-parallel-mainline-multi-account-trial-window-runbook-v1.md`
...
- [x] 发起并行主链多账号自动登录受控窗口演练后收口评审（A44）
  ...
  输入包：`design/2026-03-29-parallel-mainline-multi-account-trial-window-post-drill-close-review-prep-v1.md`
  预评审纪要：`design/2026-03-29-parallel-mainline-multi-account-trial-window-post-drill-close-review-office-hours-minutes-v1.md`
...
- [x] 执行并行主链多账号自动登录受控窗口常态运行首轮值守复核（A46）
...
- [x] 固化并行主链多账号自动登录受控窗口常态策略稳定态包（A50）
  ...
  稳定态包：`design/2026-03-29-parallel-mainline-multi-account-trial-window-steady-strategy-package-v1.md`
