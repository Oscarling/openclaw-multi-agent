...
- [ ] 执行 `rh_t5_b01_route_parity_remediation_requested`（路由口径整改）
  ...
  当前状态：A1/A3/A4/A5/A6/A7/A8 均未消除分裂；A2 完成关键脚本护栏硬化；A7 已确认分裂与 provider/model/profile 受控对比无关；A8 已确认分裂集中在 `--to` 路径；A9 已完成上游问题单升级；A10 已完成上游反馈复检包基线留痕（`blocker_close_ready=no`）；A11 已完成上游反馈探针基线留痕（`next_event=waiting_upstream_feedback`），本地追踪 issue #37 已纠偏重开
